...
class FfmpegMediaEditorService(
    ...
    override fun createTmpVideo(audioFile: Path, subtitleFile: Path, resultFile: Path): Uni<Path> {
        ...
        return this.commandLineExecutor.executeCommand(command = command)
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Ffmpeg failed creating tmp video - exit value: ${e.exitValue} - audio file: ${audioFile.absolutePathString()} - result file: ${resultFile.absolutePathString()}", e)
    ...
    override fun createFinalVideo(originalVideoFile: Path, newVideoFile: Path, resultFile: Path): Uni<Path> {
        return Uni
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Ffmpeg failed creating final video - exit value: ${e.exitValue} - original video file: ${originalVideoFile.absolutePathString()} - new video file: ${newVideoFile.absolutePathString()} - result file: ${resultFile.absolutePathString()}", e)
    ...
    override fun getDuration(file: Path): Uni<Duration> {
        ...
        return this.commandLineExecutor.executeCommand(command = command)
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Ffmpeg failed getting duration of file - exit value: ${e.exitValue} - file: ${file.absolutePathString()}", e)
    ...
    override fun concatFiles(files: List<Path>, resultFile: Path, interleaveFile: Path?): Uni<Path> {
        ...
        return this.writeConcatFile(files, interleaveFile)
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Ffmpeg failed concatenating files - exit value: ${e.exitValue} - files: ${files.map { it.absolutePathString() }} - result file: ${resultFile.absolutePathString()} - interleave file: ${interleaveFile?.absolutePathString()}", e)
