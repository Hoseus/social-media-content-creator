...
class MfaSubtitleService(
    ...
    override fun generateSubtitles(audioFile: Path, transcriptFile: Path, resultFile: Path): Uni<Path> {
        ...
        .onFailure().invoke { e ->
            when(e) {
                is CommandLineExecutionFailedException -> logger.error("Mfa failed alignment - exit value: ${e.exitValue} - audio file: ${audioFile.absolutePathString()} - transcript file: ${transcriptFile.absolutePathString()} - result file: ${resultFile.absolutePathString()}", e)
    ...
    override fun batchGenerateSubtitles(audioFiles: List<Path>, transcriptFiles: List<Path>, resultFileProvider: (Int, Path, Path) -> Path): Uni<List<Path>> {
        ...
        return this.fileSystem.mkdir(tmpDir.absolutePathString()).flatMap {
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Mfa failed alignment - exit value: ${e.exitValue} - audio files: ${audioFiles.map { it.absolutePathString() }} - transcript files: ${transcriptFiles.map { it.absolutePathString() }} - result file provider: $resultFileProvider", e)
