...
class CoquiTtsService(
    ...
    override fun tts(text: String, speakerSex: SpeakerSex, resultFile: Path): Uni<Path> {
        ...
        return this.commandLineExecutor.executeCommand(command = command)
            ...
            .onFailure().invoke { e ->
                when(e) {
                    is CommandLineExecutionFailedException -> logger.error("Coqui tts failed to synthesize speech - exit value: ${e.exitValue} - text: \"$text\" - result file: ${resultFile.absolutePathString()}", e)
